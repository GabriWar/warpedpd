...
public class DesktopLaunchValidator {
	...
	public static boolean verifyValidJVMState(String[] args){
		...
		if (SharedLibraryLoader.os == Os.MacOsX){
			...
			if ("true".equals(System.getProperty("wpdRelaunched"))){
			...
			jvmArgs.add("-DwpdRelaunched=true");
			jvmArgs.addAll(ManagementFactory.getRuntimeMXBean().getInputArguments());
			jvmArgs.add("-cp");
			jvmArgs.add(System.getProperty("java.class.path"));
			jvmArgs.add(DesktopLauncher.class.getName());

			System.err.println("Error: WarpedPD must start on the first thread in order to work on macOS.");
