...
public class DesktopPlatformSupport extends PlatformSupport {
	...
	public void updateDisplaySize() {
		if (previousSizes == null){
			...
			previousSizes[1] = WPDSettings.windowResolution();
		} else {
			previousSizes[1] = previousSizes[0];
		}
		previousSizes[0] = new Point(Game.width, Game.height);
		if (!WPDSettings.fullscreen()) {
			WPDSettings.windowResolution( previousSizes[0] );
		}
	}
			...
			public void run () {
				if (WPDSettings.fullscreen()){
					...
					if (!first){
						...
						monitorNum = WPDSettings.fulLScreenMonitor();
					}
					...
					WPDSettings.fulLScreenMonitor(monitorNum);
				} else {
