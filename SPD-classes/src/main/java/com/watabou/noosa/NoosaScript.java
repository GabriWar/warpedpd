...
public class NoosaScript extends Script {
	...
	//The index buffer is 16-bit, so a single draw command can only address
	// 65536 vertices (16384 quads). Sets larger than that (huge tilemaps) are
	// drawn in chunks, re-pointing the vertex attributes at each chunk so
	// index values stay in range.
	public void drawQuadSetLarge( Vertexbuffer buffer, int size ){

		if (size == 0) {
			return;
		}

		final int LIMIT = 16384;

		buffer.updateGLData();

		for (int offset = 0; offset < size; offset += LIMIT){

			int length = Math.min( LIMIT, size - offset );

			buffer.bind();

			aXY.vertexBuffer( 2, 4, offset*16 );
			aUV.vertexBuffer( 2, 4, offset*16 + 2 );
