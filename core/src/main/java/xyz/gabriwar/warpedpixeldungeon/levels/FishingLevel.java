...
public class FishingLevel extends Level {
	...
	protected boolean build() {
		...
		// FishingLevel: flood most empty tiles with water
		for (int i = 0; i < length(); i++) {
			if (map[i] == Terrain.EMPTY && Random.Float() < .95f) {
				map[i] = Terrain.WATER;
			}
		}
	...
	protected void createItems() {
		...
		if (entrance != null) {
			int pos = entrance.cell() + 1;
			FishingBomb bombs = new FishingBomb();
			bombs.quantity(99);
			drop(bombs, pos).type = Heap.Type.CHEST;
	...
	public int randomRespawnCell( Char ch ) {
		//fish live in the water: this is the single placement hook both the initial
		//createMobs() loop and the modern MobSpawner respawn path funnel through, so
		//keeping it water-only fixes both at once. Bails to -1 so spawnMob stays sane.
		int cell;
		int tries = 30;
		do {
			if (tries-- <= 0) return -1;
			cell = Random.Int(length());
		} while (map[cell] != Terrain.WATER || Actor.findChar(cell) != null);
		return cell;
	}
}
