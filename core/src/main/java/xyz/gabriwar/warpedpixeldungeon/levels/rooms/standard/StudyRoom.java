...
import xyz.gabriwar.warpedpixeldungeon.items.scrolls.exotic.ScrollOfMetamorphosis;
...
public class StudyRoom extends StandardRoom {
	...
	public void paint(Level level) {
		...
		if (prize == null) {
			//living-world twist: the observatory reads the sky. An aurora makes
			//the northern script legible; a solar eclipse unlocks the black codex;
			//bright daylight lets you pre-read the shelved scroll before taking it.
			if (ClimateManager.isAurora()) {
				prize = new ScrollOfTransmutation();
			} else if (ClimateManager.isSolarEclipse()) {
				prize = new ScrollOfMetamorphosis();
			} else {
				prize = Generator.random( Random.oneOf(
						Generator.Category.POTION,
						Generator.Category.SCROLL));
				if (DayNightCycle.isBright()) {
