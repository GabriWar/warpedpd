...
import xyz.gabriwar.warpedpixeldungeon.items.journal.GuidePage;
import xyz.gabriwar.warpedpixeldungeon.items.journal.RegionLorePage;
...
import xyz.gabriwar.warpedpixeldungeon.levels.builders.LoopBuilder;
...
import xyz.gabriwar.warpedpixeldungeon.levels.rooms.special.PitRoom;
import xyz.gabriwar.warpedpixeldungeon.levels.rooms.special.SacrificeRoom;
import xyz.gabriwar.warpedpixeldungeon.levels.rooms.special.ShopRoom;
import xyz.gabriwar.warpedpixeldungeon.levels.rooms.special.SpecialRoom;
import xyz.gabriwar.warpedpixeldungeon.levels.rooms.special.StatueRoom;
import xyz.gabriwar.warpedpixeldungeon.levels.rooms.standard.StandardRoom;
import xyz.gabriwar.warpedpixeldungeon.levels.rooms.standard.entrance.EntranceRoom;
import xyz.gabriwar.warpedpixeldungeon.levels.rooms.standard.exit.ExitRoom;
...
public abstract class RegularLevel extends Level {
	...
	public int mobLimit() {
		...
		//in phase with standardRooms(): floors grow through each region, mobs follow
		int mobs = 4 + (Dungeon.depth - 1) % 5 + Random.Int(3);
		if (feeling == Feeling.LARGE){
	...
	protected void createItems() {
		...
		if (Random.Int(50) == 0) {
			...
			drop(new SteelHoneypot(), cell).type = Heap.Type.HEAP;
		}
