...
import xyz.gabriwar.warpedpixeldungeon.levels.templeChambers.AlchemyChamber;
import xyz.gabriwar.warpedpixeldungeon.levels.templeChambers.Chamber;
import xyz.gabriwar.warpedpixeldungeon.levels.templeChambers.ConfusionChamber;
import xyz.gabriwar.warpedpixeldungeon.levels.templeChambers.MazeChamber;
import xyz.gabriwar.warpedpixeldungeon.levels.templeChambers.MimicInTheGrassChamber;
import xyz.gabriwar.warpedpixeldungeon.levels.templeChambers.MineFieldChamber;
import xyz.gabriwar.warpedpixeldungeon.levels.templeChambers.MoistureChamber;
import xyz.gabriwar.warpedpixeldungeon.levels.templeChambers.PiranhaPoolChamber;
import xyz.gabriwar.warpedpixeldungeon.levels.templeChambers.SentryChamber;
import xyz.gabriwar.warpedpixeldungeon.levels.templeChambers.SentryMazeChamber;
import xyz.gabriwar.warpedpixeldungeon.levels.templeChambers.SpearGnollChamber;
import xyz.gabriwar.warpedpixeldungeon.levels.templeChambers.WarpStoneChamber;
...
//                this.drop(mainLoot, remains).setHauntedIfCursed().type = Heap.Type.SKELETON;
        ...
        //RA dropped its OldAmulet here, but that item is bound to hero classes we
        //are not porting. The gauntlet instead pays out a guaranteed upgrade scroll.
        this.drop(new ScrollOfUpgrade(), amuletCell());
    ...
    //RA set this when the OldAmulet was taken. That item is class-bound and unported,
    //so the gauntlet is marked complete once the hero actually reaches the prize.
    @Override
    public void occupyCell( Char ch ) {
        super.occupyCell( ch );
        if (ch == Dungeon.hero && !Dungeon.templeCompleted
                && Dungeon.level.distance( ch.pos, amuletCell() ) <= 1) {
