...
public class ZotBossLevel extends Level {
	...
	//the arena's row of summoning traps calls this. Returning null made every one of them
	//inert, so the fight was fought in an empty room. createMobs() stays empty on purpose -
	//nothing spawns here on its own, only what the traps pull in.
	@Override
