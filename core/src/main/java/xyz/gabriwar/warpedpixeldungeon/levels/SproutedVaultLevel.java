...
public class SproutedVaultLevel extends Level {
	...
	protected boolean build() {
		...
		// Decoration (Sprouted had a separate decorate() method)
		...
		for (int i = 0; i < length(); i++) {
			...
			if (map[i] == Terrain.EMPTY && heaps.get(i) == null && Random.Float() < .15f) {
				map[i] = Terrain.STATUE;
			}
			if (map[i] == Terrain.EMPTY && heaps.get(i) == null && Random.Float() < .10f) {
	...
	protected void createItems() {
		for (int i = 0; i < 50; i++) {
			...
			if (pos != -1) {
				drop(new Gold(Random.IntRange(100, 300)), pos);
			}
		}
	}
