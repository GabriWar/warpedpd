...
public abstract class SpecialRoom extends Room {
	...
	private static final ArrayList<Class<? extends SpecialRoom>> CONSUMABLE_SPECIALS = new ArrayList<>( Arrays.asList(
			...
			TrapsRoom.class, CrystalPathRoom.class, EnchantingLibraryRoom.class
	) );
	...
	public static void initForRun() {
		...
		// Interleave equip and consumable specials evenly (10 each)
	...
	public static SpecialRoom createRoom(){
		...
		//guaranteed enchanting pedestal on the first floor after the dwarf king
		} else if (Dungeon.depth == 21 && Dungeon.branch == 0
				&& floorSpecials.contains(EnchantingLibraryRoom.class)) {

			useType(EnchantingLibraryRoom.class);
			return new EnchantingLibraryRoom();
	...
	public static void restoreRoomsFromBundle( Bundle bundle ) {
		...
		if (bundle.contains( ROOMS )) {
			...
			initForRun();
			WarpedPixelDungeon.reportException(new Exception("specials array didn't exist!"));
		}
