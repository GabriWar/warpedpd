...
import xyz.gabriwar.warpedpixeldungeon.items.remains.RemainsItem;
...
public class Badges {
	...
	public static void validateItemsCrafted() {
		...
		//legacy Sprouted potions cooked badges (use itemsCrafted counter)
		Badge potBadge = null;
		if (!local.contains( Badge.POTIONS_COOKED_1 ) && Statistics.itemsCrafted >= 3) {
			potBadge = Badge.POTIONS_COOKED_1;
			local.add( potBadge );
		}
		if (!local.contains( Badge.POTIONS_COOKED_2 ) && Statistics.itemsCrafted >= 6) {
			if (potBadge != null) unlock(potBadge);
			potBadge = Badge.POTIONS_COOKED_2;
			local.add( potBadge );
		}
		if (!local.contains( Badge.POTIONS_COOKED_3 ) && Statistics.itemsCrafted >= 9) {
			if (potBadge != null) unlock(potBadge);
			potBadge = Badge.POTIONS_COOKED_3;
			local.add( potBadge );
		}
		if (!local.contains( Badge.POTIONS_COOKED_4 ) && Statistics.itemsCrafted >= 12) {
			if (potBadge != null) unlock(potBadge);
			potBadge = Badge.POTIONS_COOKED_4;
			local.add( potBadge );
		}
		displayBadge( potBadge );
	}
	...
	public static boolean checkOrbObtained() {
		return global != null && global.contains(Badge.ORB);
	}
