...
import xyz.gabriwar.warpedpixeldungeon.ui.ItemJournalButton;
...
public class WndUseItem extends WndInfoItem {
	...
	public WndUseItem( final Window owner, final Item item ) {
		...
		if (Dungeon.hero.isAlive() && Dungeon.hero.belongings.contains(item)) {
			...
			for (final String action : item.actions(Dungeon.hero)) {
				...
				RedButton btn = new RedButton(item.actionName(action, Dungeon.hero), 8) {
					...
					protected void onClick() {
						...
						if (Dungeon.hero.isAlive() && Dungeon.hero.belongings.contains(item)) {
							xyz.gabriwar.warpedpixeldungeon.items.Item.executeNetAware(item, Dungeon.hero, action);
						}
			...
			IconButton renameBtn = new IconButton(Icons.get(Icons.SCROLL_GREY)) {
				@Override
				protected void onClick() {
					GameScene.show(new WndTextInput(
							Messages.get(WndUseItem.class, "rename_title"),
							Messages.get(WndUseItem.class, "rename_body"),
							item.customName != null ? item.customName : "",
							20, false,
							Messages.get(WndUseItem.class, "rename_confirm"),
							Messages.get(WndUseItem.class, "rename_cancel")
					) {
						@Override
						public void onSelect(boolean positive, String text) {
							if (positive) {
								item.customName = text.trim().isEmpty() ? null : text.trim();
								Item.updateQuickslot();
							}
						}
					});
			...
			renameBtn.setRect(width - 32, 0, 16, 16);
			add(renameBtn);
		}
