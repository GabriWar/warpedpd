...
public class WndSettings extends WndTabbed {
	...
	public void hide() {
		...
		WarpedPixelDungeon.seamlessResetScene(new Game.SceneChangeCallback() {
			@Override
		...
		protected void createChildren() {
			...
			chkFullscreen = new CheckBox( fullscreenText ) {
				...
				protected void onClick() {
					super.onClick();
					WPDSettings.fullscreen(checked());
				}
			};
			if (Game.platform.supportsFullScreen()){
				chkFullscreen.checked(WPDSettings.fullscreen());
			} else {
			...
			if (DeviceCompat.isAndroid()) {
				chkLandscape = new CheckBox(Messages.get(this, "landscape")) {
					...
					protected void onClick() {
						super.onClick();
						WPDSettings.landscape(checked());
					}
				};
				chkLandscape.checked(WPDSettings.landscape());
				add(chkLandscape);
			}
			...
			optBrightness = new OptionSlider(Messages.get(this, "brightness"),
				...
				protected void onChange() {
					WPDSettings.brightness(getSelectedValue());
				}
			};
			optBrightness.setSelectedValue(WPDSettings.brightness());
			add(optBrightness);
			...
			optVisGrid = new OptionSlider(Messages.get(this, "visual_grid"),
				...
				@Override
				protected void onChange() {
					WPDSettings.visualGrid(getSelectedValue());
				}
			};
			optVisGrid.setSelectedValue(WPDSettings.visualGrid());
			add(optVisGrid);
			...
			optFollowIntensity = new OptionSlider(Messages.get(this, "camera_follow"),
				...
				@Override
				protected void onChange() {
					WPDSettings.cameraFollow(getSelectedValue());
				}
			};
			optFollowIntensity.setSelectedValue(WPDSettings.cameraFollow());
			...
			optScreenShake = new OptionSlider(Messages.get(this, "screenshake"),
				...
				@Override
				protected void onChange() {
					WPDSettings.screenShake(getSelectedValue());
				}
			};
			optScreenShake.setSelectedValue(WPDSettings.screenShake());
			add(optScreenShake);
	...
	private static class UITab extends Component {
		...
		protected void createChildren() {
			...
			if (Math.min(wMin, hMin) >= 2*Game.density){
				optUIMode = new OptionSlider(
					...
					@Override
					protected void onChange() {
						WPDSettings.interfaceSize(getSelectedValue());
						WarpedPixelDungeon.seamlessResetScene();
					}
				};
				optUIMode.setSelectedValue(WPDSettings.interfaceSize());
				add(optUIMode);
			}
			...
			if ((int)Math.ceil(2* Game.density) < PixelScene.maxDefaultZoom) {
				optUIScale = new OptionSlider(Messages.get(this, "scale"),
					...
					@Override
					protected void onChange() {
						if (getSelectedValue() != WPDSettings.scale()) {
							WPDSettings.scale(getSelectedValue());
							WarpedPixelDungeon.seamlessResetScene();
						}
					}
				};
...

							{
								...
								btnSplit = new RedButton(Messages.get(WndSettings.UITab.this, "split")) {
									...
									protected void onClick() {
										...
										WPDSettings.toolbarMode(Toolbar.Mode.SPLIT.name());
										Toolbar.updateLayout();
									}
								};
								if (WPDSettings.toolbarMode().equals(Toolbar.Mode.SPLIT.name())) {
								...
								btnGrouped = new RedButton(Messages.get(WndSettings.UITab.this, "group")) {
									...
									protected void onClick() {
										...
										textColor(TITLE_COLOR);
										btnCentered.textColor(WHITE);
										WPDSettings.toolbarMode(Toolbar.Mode.GROUP.name());
										Toolbar.updateLayout();
									}
								};
								if (WPDSettings.toolbarMode().equals(Toolbar.Mode.GROUP.name())) {
								...
								btnCentered = new RedButton(Messages.get(WndSettings.UITab.this, "center")) {
									...
									protected void onClick() {
										...
										btnGrouped.textColor(WHITE);
										textColor(TITLE_COLOR);
										WPDSettings.toolbarMode(Toolbar.Mode.CENTER.name());
										Toolbar.updateLayout();
									}
								};
								if (WPDSettings.toolbarMode().equals(Toolbar.Mode.CENTER.name())) {
								...
								chkQuickSwapper = new CheckBox(Messages.get(WndSettings.UITab.this, "quickslot_swapper")) {
									...
									protected void onClick() {
										super.onClick();
										WPDSettings.quickSwapper(checked());
										Toolbar.updateLayout();
									}
								};
								chkQuickSwapper.checked(WPDSettings.quickSwapper());
								add(chkQuickSwapper);
								...
								chkFlipToolbar = new CheckBox(Messages.get(WndSettings.UITab.this, "flip_toolbar")) {
									...
									protected void onClick() {
										super.onClick();
										WPDSettings.flipToolbar(checked());
										Toolbar.updateLayout();
									}
								};
								chkFlipToolbar.checked(WPDSettings.flipToolbar());
								add(chkFlipToolbar);
								...
								chkFlipTags = new CheckBox(Messages.get(WndSettings.UITab.this, "flip_indicators")){
									...
									protected void onClick() {
										super.onClick();
										WPDSettings.flipTags(checked());
										GameScene.layoutTags();
									}
								};
								chkFlipTags.checked(WPDSettings.flipTags());
								add(chkFlipTags);
			...
			chkSundial = new CheckBox(Messages.get(this, "sundial")) {
				@Override
				protected void onClick() {
					super.onClick();
					WPDSettings.sundial(checked());
				}
			};
			chkSundial.checked(WPDSettings.sundial());
			add(chkSundial);

			if (DeviceCompat.isDebug() || WPDSettings.debugUnlocked()) {
				chkInspector = new CheckBox(Messages.get(this, "inspector")) {
					@Override
					protected void onClick() {
						super.onClick();
						WPDSettings.inspector(checked());
					}
				};
				chkInspector.checked(WPDSettings.inspector());
				add(chkInspector);
			}
			...
			chkFont = new CheckBox(Messages.get(this, "system_font")){
				...
				protected void onClick() {
					...
					WarpedPixelDungeon.seamlessResetScene(new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							WPDSettings.systemFont(checked());
						}
			...
			chkFont.checked(WPDSettings.systemFont());
			add(chkFont);
			...
			if (chkVibrate.active) {
				chkVibrate.checked(WPDSettings.vibration());
			}
			add(chkVibrate);
		}
		...
		protected void layout() {
			...
			chkSundial.setRect(0, height + GAP, width, BTN_HEIGHT);
			height = chkSundial.bottom();

			if (chkInspector != null) {
				chkInspector.setRect(0, height + GAP, width, BTN_HEIGHT);
				height = chkInspector.bottom();
			}
		...
		protected void createChildren() {
			...
			if (DeviceCompat.hasHardKeyboard()){
				...
				btnKeyBindings = new RedButton(Messages.get(this, "key_bindings")){
					...
					protected void onClick() {
						super.onClick();
						WarpedPixelDungeon.scene().addToFront(new WndKeyBindings(false));
					}
				};
			...
			if (ControllerHandler.isControllerConnected()){
				btnControllerBindings = new RedButton(Messages.get(this, "controller_bindings")){
					...
					protected void onClick() {
						super.onClick();
						WarpedPixelDungeon.scene().addToFront(new WndKeyBindings(true));
					}
				};
			...
			optControlSens = new OptionSlider(
				...
				@Override
				protected void onChange() {
					WPDSettings.controllerPointerSensitivity(getSelectedValue());
				}
			};
			optControlSens.setSelectedValue(WPDSettings.controllerPointerSensitivity());
			add(optControlSens);
			...
			optHoldMoveSens = new OptionSlider(
				...
				@Override
				protected void onChange() {
					WPDSettings.movementHoldSensitivity(getSelectedValue());
				}
			};
			optHoldMoveSens.setSelectedValue(WPDSettings.movementHoldSensitivity());
			add(optHoldMoveSens);
		}
	...
	private static class DataTab extends Component{
		...
		protected void createChildren() {
			...
			chkNews = new CheckBox(Messages.get(this, "news")){
				...
				protected void onClick() {
					super.onClick();
					WPDSettings.news(checked());
					News.clearArticles();
				}
			};
			chkNews.checked(WPDSettings.news());
			add(chkNews);
			...
			if (Updates.supportsUpdates() && Updates.supportsUpdatePrompts()) {
				chkUpdates = new CheckBox(Messages.get(this, "updates")) {
					...
					protected void onClick() {
						super.onClick();
						WPDSettings.updates(checked());
						Updates.clearUpdate();
					}
				};
				chkUpdates.checked(WPDSettings.updates());
				add(chkUpdates);
				...
				if (Updates.supportsBetaChannel()){
					chkBetas = new CheckBox(Messages.get(this, "betas")) {
						...
						protected void onClick() {
							super.onClick();
							WPDSettings.betas(checked());
							Updates.clearUpdate();
						}
					};
					chkBetas.checked(WPDSettings.betas());
					add(chkBetas);
				}
			}
			...
			if (!DeviceCompat.isDesktop()){
				chkWifi = new CheckBox(Messages.get(this, "wifi")){
					...
					protected void onClick() {
						super.onClick();
						WPDSettings.WiFi(checked());
					}
				};
				chkWifi.checked(WPDSettings.WiFi());
				add(chkWifi);
			}
			...
			mpTitle = PixelScene.renderTextBlock("Multiplayer", 9);
			mpTitle.hardlight(TITLE_COLOR);
			add(mpTitle);

			String currentName = WPDSettings.multiplayerName();
			btnMpName = new RedButton(currentName.isEmpty() ? "Set Name" : "Name: " + currentName) {
				@Override
				protected void onClick() {
		...
		protected void layout() {
			...
			mpTitle.setPos((width - mpTitle.width()) / 2, pos + GAP);
			pos = mpTitle.bottom() + GAP;

			btnMpName.setRect(0, pos + GAP, width, BTN_HEIGHT);
			pos = btnMpName.bottom();
	...
	private static class AudioTab extends Component {
		...
		protected void createChildren() {
			...
			optMusic = new OptionSlider(Messages.get(this, "music_vol"), "0", "10", 0, 10) {
				@Override
				protected void onChange() {
					WPDSettings.musicVol(getSelectedValue());
				}
			};
			optMusic.setSelectedValue(WPDSettings.musicVol());
			add(optMusic);
			...
			chkMusicMute = new CheckBox(Messages.get(this, "music_mute")){
				...
				protected void onClick() {
					super.onClick();
					WPDSettings.music(!checked());
				}
			};
			chkMusicMute.checked(!WPDSettings.music());
			add(chkMusicMute);
			...
			optSFX = new OptionSlider(Messages.get(this, "sfx_vol"), "0", "10", 0, 10) {
				@Override
				protected void onChange() {
					WPDSettings.SFXVol(getSelectedValue());
					if (Random.Int(100) == 0){
			...
			optSFX.setSelectedValue(WPDSettings.SFXVol());
			add(optSFX);
			...
			if (DeviceCompat.isiOS()){
				...
				chkIgnoreSilent = new CheckBox( Messages.get(this, "ignore_silent") ){
					...
					protected void onClick() {
						super.onClick();
						WPDSettings.ignoreSilentMode(checked());
					}
				};
				chkIgnoreSilent.checked(WPDSettings.ignoreSilentMode());
				add(chkIgnoreSilent);
				...
				chkMusicBG = new CheckBox( Messages.get(this, "music_bg") ){
					...
					protected void onClick() {
						super.onClick();
						WPDSettings.playMusicInBackground(checked());
					}
				};
				chkMusicBG.checked(WPDSettings.playMusicInBackground());
				add(chkMusicBG);
			}
		}
				...
				RedButton btn = new RedButton(Messages.titleCase(langs.get(i).nativeName()), 6){
					...
					protected void onClick() {
						...
						WarpedPixelDungeon.seamlessResetScene(new Game.SceneChangeCallback() {
							@Override
							public void beforeCreate() {
								WPDSettings.language(langs.get(langIndex));
								GameLog.wipe();
								Game.platform.resetGenerators();
							}
							@Override
			...
			if (currLang != Languages.ENGLISH) {
				...
				btnCredits = new RedButton(credText, credText.length() > 9 ? 6 : 9) {
					...
					protected void onClick() {
						...
						WarpedPixelDungeon.scene().addToFront(credits);
					}
				};
				add(btnCredits);
			}
