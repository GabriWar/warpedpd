...
public class WndRanking extends WndTabbed {
	...
	private class StatsTab extends Group {
		...
		public StatsTab() {
			...
			} else if (record.heroName != null) {
				heroClass = record.heroName;
			} else {
				heroClass = record.heroClass.name();
			}
			...
			if (Dungeon.hero == null){
				...
				IconButton scoreInfo = new IconButton(Icons.get(Icons.INFO)) {
					...
					protected void onClick() {
						super.onClick();
						WarpedPixelDungeon.scene().addToFront(new WndScoreBreakdown());
					}
				};
			...
			if (Dungeon.hero != null && Dungeon.seed != -1 && !Dungeon.daily &&
				...
				RedButton btnSeed = new RedButton(Messages.get(this, "copy_seed")){
					...
					protected void onClick() {
						super.onClick();
						WarpedPixelDungeon.scene().addToFront(new WndOptions(new Image(icon),
							...
							protected void onSelect(int index) {
								...
								if (index == 0){
									WPDSettings.customSeed(DungeonSeed.convertToCode(Dungeon.seed));
				...
				if (DungeonSeed.convertFromText(WPDSettings.customSeed()) == Dungeon.seed){
					icon.hardlight(1f, 1.5f, 0.67f);
				}
				btnSeed.icon(icon);
