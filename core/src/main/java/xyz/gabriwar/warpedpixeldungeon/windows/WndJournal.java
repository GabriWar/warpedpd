...
import xyz.gabriwar.warpedpixeldungeon.ui.BadgesGrid;
import xyz.gabriwar.warpedpixeldungeon.ui.BadgesList;
import xyz.gabriwar.warpedpixeldungeon.ui.CustomNoteButton;
import xyz.gabriwar.warpedpixeldungeon.ui.Icons;
import xyz.gabriwar.warpedpixeldungeon.ui.QuickRecipe;
import xyz.gabriwar.warpedpixeldungeon.ui.RedButton;
import xyz.gabriwar.warpedpixeldungeon.ui.RenderedTextBlock;
import xyz.gabriwar.warpedpixeldungeon.ui.ScrollPane;
import xyz.gabriwar.warpedpixeldungeon.ui.ScrollingGridPane;
...
public class WndJournal extends WndTabbed {
	...
	public WndJournal(){
		...
		Tab[] tabs = {
				...
				//the Descent Guide is a fullscreen scene, not a pane: selecting this tab
				//closes the journal and opens it, returning to the game on back. it never
				//claims last_index, so reopening the journal doesn't relaunch the scene.
				new IconTab( Icons.MAGNIFY.get() ) {
					protected void select( boolean value ) {
						super.select( value );
						if (value){
							hide();
	...
	public boolean onSignal(KeyEvent event) {
		if (event.pressed && KeyBindings.getActionForKey( event ) == WPDAction.JOURNAL) {
			onBackPressed();
			return true;
		} else {
			...
			for (String page : Document.ADVENTURERS_GUIDE.pageNames()){
				...
				ScrollingListPane.ListItem item = new ScrollingListPane.ListItem(
					...
					public boolean onClick(float x, float y) {
						if (inside( x, y ) && found) {
							WarpedPixelDungeon.scene().addToFront( new WndStory( Document.ADVENTURERS_GUIDE.pageSprite(page),
				...
				public boolean onClick(float x, float y) {
					if (inside(x, y)) {
						...
						if (WarpedPixelDungeon.scene() instanceof GameScene){
							...
							WarpedPixelDungeon.scene().addToFront(new WndJournalItem(sprite, finalTitle, finalDesc));
						}
						return true;
					} else {
						return false;
	...
	private static void addGridEntities(ScrollingGridPane grid, Collection<Class<?>> classes) {
		for (Class<?> entityCls : classes){
			...
			String finalTitle = title;
			String finalDesc = desc;
			ArrayList<Mob.DropInfo> finalDrops = drops;
			ScrollingGridPane.GridItem gridItem = new ScrollingGridPane.GridItem(icon) {
				...
				public boolean onClick(float x, float y) {
					if (inside(x, y)) {
						...
						if (WarpedPixelDungeon.scene() instanceof GameScene){
							GameScene.show(new WndJournalItem(image, finalTitle, finalDesc, finalDrops));
						} else {
							WarpedPixelDungeon.scene().addToFront(new WndJournalItem(image, finalTitle, finalDesc, finalDrops));
						}
					...
					if (inside(x, y)) {
						...
						if (seen) {
							if (WarpedPixelDungeon.scene() instanceof GameScene){
								...
								WarpedPixelDungeon.scene().addToFront(new WndStory(sprite, doc.pageTitle(page), doc.pageBody(page)));
							}
							...
							if (WarpedPixelDungeon.scene() instanceof GameScene){
								...
								WarpedPixelDungeon.scene().addToFront(new WndJournalItem(sprite, "???",
										Messages.get(CatalogTab.class, "not_seen_lore") + "\n\n" + doc.discoverHint()));
							}
