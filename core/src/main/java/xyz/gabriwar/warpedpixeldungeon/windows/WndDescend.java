...
public class WndDescend extends Window {
	...
	public WndDescend() {
		...
		RedButton btnDescend = new RedButton( Messages.get(this, "descend") ) {
			@Override
		...
		btnDescend.setRect( 0, message.top() + message.height() + GAP, WIDTH, BTN_HEIGHT );
		add( btnDescend );

		resize( WIDTH, (int) btnDescend.bottom() );
	}
}
