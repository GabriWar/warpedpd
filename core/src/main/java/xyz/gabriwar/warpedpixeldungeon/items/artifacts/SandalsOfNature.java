...
public class SandalsOfNature extends Artifact {
	...
	public ArrayList<String> actions( Hero hero ) {
		...
		if (isEquipped( hero )
				&& !cursed
				&& curSeedEffect != null
				&& charge >= seedChargeReqs.getOrDefault(curSeedEffect, 20)) {
			actions.add(AC_ROOT);
		}
		return actions;
	}
	...
	public void execute( Hero hero, String action ) {
		...
		if (action.equals(AC_FEED)){
			...
			else if (charge < seedChargeReqs.getOrDefault(curSeedEffect, 20))    GLog.i( Messages.get(this, "low_charge") );
			else {
	...
	public ItemSprite.Glowing glowing() {
		if (curSeedEffect != null){
			return new ItemSprite.Glowing(seedColors.getOrDefault(curSeedEffect, 0x66BB44));
		}
		return null;
	}
	...
	public String desc() {
		...
		if (curSeedEffect != null){
				desc += "\n\n" + Messages.get(this, "desc_ability",
					...
					seedChargeReqs.getOrDefault(curSeedEffect, 20));
		}
		...
		public void onSelect(Integer cell) {
			if (cell != null){
				...
				if (!Dungeon.level.heroFOV[cell] || Dungeon.level.distance(curUser.pos, cell) > 3){
					...
					Splash.at(DungeonTilemap.tileCenterToWorld( cell ), -PointF.PI/2, PointF.PI/2, seedColors.getOrDefault(curSeedEffect, 0x66BB44), 6);
