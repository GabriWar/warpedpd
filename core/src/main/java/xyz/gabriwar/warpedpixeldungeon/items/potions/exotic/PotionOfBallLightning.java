...
public class PotionOfBallLightning extends ExoticPotion {
	...
	{
		icon = ItemSpriteSheet.Icons.POTION_BALLLIGHTNING;
	}
	...
	private void arc(Char ch) {
		affected.add(ch);
		int dist;
		if (Dungeon.level.water[ch.pos] && !ch.flying) dist = 2;
		else dist = 1;
		PathFinder.buildDistanceMap(ch.pos, BArray.not(Dungeon.level.solid, null), dist);
		for (int i = 0; i < PathFinder.distance.length; i++) {
			if (PathFinder.distance[i] < Integer.MAX_VALUE) {
				Char n = Actor.findChar(i);
				if (n == Dungeon.hero && PathFinder.distance[i] > 1) continue;
	...
	public void apply(Hero hero) {
		...
		for (int i = 10; i > 0; i--) {
			...
			if (hero.fieldOfView[c] && !cells.contains(c)) cells.add(c);
		}
		for (int p : cells) {
