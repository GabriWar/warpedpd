...
public class TenguKey extends Item {
	...
	public void execute(Hero hero, String action) {
		...
		if (action.equals(AC_PORT)) {
			...
			//only block LEAVING the den before its boss is dead — elsewhere the key
			//ports you TO the den. (The port shipped this inverted, blocking all use.)
			...
			//no porting out of a boss arena. Without this the RETURN branch below fires
			//on depths 5/10/15/20/25 with returnDepth still -1, which sets Dungeon.depth
			//to -1 and lands the hero in a DeadEndLevel with no way back.
