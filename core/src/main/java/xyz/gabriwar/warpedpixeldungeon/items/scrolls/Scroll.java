...
import xyz.gabriwar.warpedpixeldungeon.items.scrolls.exotic.ScrollOfAntiMagic;
...
public abstract class Scroll extends Item {
	...
	private static final LinkedHashMap<String, Integer> runes = new LinkedHashMap<String, Integer>() {
		{
			...
			put("ANSUZ",ItemSpriteSheet.SCROLL_ANSUZ);
			put("HAGALAZ",ItemSpriteSheet.SCROLL_HAGALAZ);
			put("THURISAZ",ItemSpriteSheet.SCROLL_THURISAZ);
		}
	};
	...
	public void execute( Hero hero, String action ) {
		...
		if (action.equals( AC_READ )) {
			...
			} else if (hero.buff(xyz.gabriwar.warpedpixeldungeon.actors.buffs.Drenched.class) != null
					&& xyz.gabriwar.warpedpixeldungeon.actors.buffs.Drenched.scrollFails()) {
				GLog.w( Messages.get(this, "wet_pages") );
			} else {
				doRead();
			}
	...
	public static class ScrollToStone extends Recipe {
		...
		public Item brew(ArrayList<Item> ingredients) {
			...
			if (WarpedPixelDungeon.scene() instanceof AlchemyScene){
				if (!s.isIdentified()){
					((AlchemyScene) WarpedPixelDungeon.scene()).showIdentify(s);
				}
			} else {
				s.identify();
			}
