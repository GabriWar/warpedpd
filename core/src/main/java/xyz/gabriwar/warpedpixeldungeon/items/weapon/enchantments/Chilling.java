...
public class Chilling extends Weapon.Enchantment {
	...
	public int proc( Weapon weapon, Char attacker, Char defender, int damage ) {
		...
		if (Random.Float() < procChance) {
			...
			float durationToAdd = 3f * powerMulti * power(); //scales with enchantment level
			Chill existing = defender.buff(Chill.class);
			if (existing != null){
				durationToAdd = Math.min(durationToAdd, (6f*powerMulti*power())-existing.cooldown()); //scales with enchantment level
			}
