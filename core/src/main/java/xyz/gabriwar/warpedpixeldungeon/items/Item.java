...
public class Item implements Bundlable {
	...
	// Sprouted: reinforced items can be upgraded past level 15
	...
	/**
	 * Net MP-aware item action dispatch. UI sites should call this instead of
	 * item.execute(...) so that in player mode mutations round-trip to the host.
	 *
	 * - DROP → PLAYER_ACTION {type:"drop", item:name}
	 * - THROW → spawn cellSelector → {type:"throw", item:name, cell:target}
	 * - any other action (DRINK/EAT/READ/EQUIP/UNEQUIP/CAST/...) →
	 *     {type:"use", item:name, action:actionName}
	 *
	 * Local-only UI actions (Bag.AC_OPEN — opens client-side window) are NOT
	 * routed; they execute locally so the player can still browse bags.
	 */
	public static void executeNetAware( final Item item, Hero hero, final String action ) {
		if (item == null) return;
		if (xyz.gabriwar.warpedpixeldungeon.net.NetManager.isPlayer()
				&& !xyz.gabriwar.warpedpixeldungeon.items.bags.Bag.AC_OPEN.equals(action)) {
			if (AC_DROP.equals(action)) {
				xyz.gabriwar.warpedpixeldungeon.net.NetManager.sendInventoryAction(
						"drop", item.name(), -1);
			} else if (AC_THROW.equals(action)) {
	...
	public static void executeNetAware( Item item, Hero hero ) {
		if (item == null) return;
		String action = item.defaultAction();
		if (action != null) executeNetAware(item, hero, action);
	}
	...
	//catalog hint for craft-only items (Re-ARranged alchemy weapons override this)
