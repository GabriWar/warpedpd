...
abstract public class MissileWeapon extends Weapon {
	...
	public int throwPos(Hero user, int dst) {
		...
		if (hasEnchant(Projecting.class, user)){
			projecting += 4;
			//scales with enchantment level: +1 throw range per 2 empowerment levels
			Enchantment proj = getEnchant(Projecting.class);
			if (proj != null) projecting += proj.level()/2;
		}
		if ((!(this instanceof SpiritBow.SpiritArrow) && Random.Int(3) < user.pointsInTalent(Talent.SHARED_ENCHANTMENT))){
			...
			if (bow != null && bow.hasEnchant(Projecting.class, user)) {
				projecting += 4;
				Enchantment proj = bow.getEnchant(Projecting.class);
				if (proj != null) projecting += proj.level()/2;
			}
		}
	...
	public int damageRoll(Char owner) {
		...
		if (owner instanceof Hero) {
			...
			// Wind: very subtle tail/headwind damage modifier (±8 % max)
			Char target = ((Hero) owner).attackTarget();
			if (target != null) {
				damage = Math.round(damage * ClimateManager.windProjectileFactor(
	...
	public Item merge(Item other) {
		...
		if (isSimilar(other)) {
			...
			//secondary enchantment: adopt the other stack's if this stack lacks one
			if (enchantment2 == null && ((MissileWeapon) other).enchantment2 != null){
				enchantment2 = ((MissileWeapon) other).enchantment2;
			}
			//matching enchantments keep the higher empowerment level
			if (enchantment != null && ((MissileWeapon) other).enchantment != null
					&& enchantment.getClass() == ((MissileWeapon) other).enchantment.getClass()){
				enchantment.level(Math.max(enchantment.level(), ((MissileWeapon) other).enchantment.level()));
			}
			if (enchantment2 != null && ((MissileWeapon) other).enchantment2 != null
					&& enchantment2.getClass() == ((MissileWeapon) other).enchantment2.getClass()){
				enchantment2.level(Math.max(enchantment2.level(), ((MissileWeapon) other).enchantment2.level()));
			}
		}
		return this;
	}
