...
public class Potential extends Glyph {
	...
	private static ItemSprite.Glowing WHITE = new ItemSprite.Glowing( 0xDDAAFF, 0.6f );
	...
	public int proc( Armor armor, Char attacker, Char defender, int damage) {
		...
		if (Random.Float() < procChance && defender instanceof Hero) {
			...
			int wands = ((Hero) defender).belongings.charge( powerMulti * power() ); //scales with glyph level
			if (wands > 0) {
