...
import xyz.gabriwar.warpedpixeldungeon.items.journal.DocumentPage;
...
public class Heap implements Bundlable {
	...
	public void burn() {
		...
		for (Item item : items.toArray( new Item[0] )) {
			if (item instanceof Scroll && !item.unique) {
				...
				items.remove( item );
				evaporated = true;
			} else if (item instanceof MysteryMeat || item instanceof MonsterMeat || item instanceof FrozenCarpaccio) {
				replace( item, ChargrilledMeat.cook( item.quantity ) );
				burnt = true;
			} else if (item.getClass() == Nut.class) {
				//fire toasts a raw nut (not a GoldenNut or an already-toasted one)
				replace( item, ToastedNut.cook( (Nut) item ) );
				burnt = true;
			} else if (item instanceof Egg) {
				//an egg left in the fire incubates rather than cooks
				((Egg) item).burns++;
				burnt = true;
	...
	// Sprouted: evaporate all dewdrops when leaving the floor
	public void dryup() {
		...
		if (evaporated) {
			if (Dungeon.level.heroFOV[pos]) evaporateFX(pos);
			if (isEmpty()) destroy();
	...
	public void freeze() {
		...
		for (Item item : items.toArray( new Item[0] )) {
			if (item instanceof MysteryMeat || item instanceof MonsterMeat) {
				replace( item, FrozenCarpaccio.cook( (Food)item ) );
				frozen = true;
	...
	//an egg lying on the floor incubates differently depending on what happens to it.
	//burn()/freeze() above cover fire and frost; these three cover the trap effects.
	public void lit() {
		incubate(Egg.Incubation.LIT);
	}

	public void summon() {
		incubate(Egg.Incubation.SUMMON);
	}

	public void poison() {
		incubate(Egg.Incubation.POISON);
	}

	private void incubate( Egg.Incubation type ) {
		if (this.type != Type.HEAP) {
			return;
		}
		for (Item item : items.toArray( new Item[0] )) {
			if (item instanceof Egg) {
				((Egg) item).incubate( type );
			}
		}
	}
