...
public class TrinketCatalyst extends Item {
	...
	public static class Recipe extends xyz.gabriwar.warpedpixeldungeon.items.Recipe {
		...
		public Item brew(ArrayList<Item> ingredients) {
			...
			WarpedPixelDungeon.scene().addToFront(new WndTrinket(newCata));
			try {
			...
			for (int i = 0; i < NUM_TRINKETS; i++){
				ItemButton btnReward = new ItemButton() {
					...
					protected void onClick() {
						WarpedPixelDungeon.scene().addToFront(new RewardWindow(item()));
					}
				};
					...
					protected void onClick() {
						...
						if (cata != null) {
							...
							if (WarpedPixelDungeon.scene() instanceof AlchemyScene) {
								((AlchemyScene) WarpedPixelDungeon.scene()).craftItem(null, result);
							} else {
