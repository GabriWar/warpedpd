...
public class MeleeWeapon extends Weapon {
	...
	// Seed coating: attacks proc the seed's effect for a limited number of hits
	...
	public String defaultAction() {
		if (Dungeon.hero != null && hasDuelistAbility()
			&& (Dungeon.hero.heroClass == HeroClass.DUELIST
			|| Dungeon.hero.hasTalent(Talent.SWIFT_EQUIP))
			&& !(this instanceof Gun) && !(this instanceof BowWeapon)){
			return AC_ABILITY;
		} else {
	...
	public ArrayList<String> actions(Hero hero) {
		...
		if (isEquipped(hero) && hero.heroClass == HeroClass.DUELIST && hasDuelistAbility()){
	...
	//weapons ported from forks with no Duelist have no ability designed for them. Without
	//this they'd still advertise AC_ABILITY, whose button renders as the missing-key
	//placeholder and whose press does nothing. Upstream hardcoded `this instanceof
	//MagesStaff` for the same reason; this generalises it.
	...
	//Re-ARranged parity: accessor used by ported weapons (guns modify effective tier)
	public int tier() {
		return this.tier;
	}
	...
	public String info() {
		...
		if (Dungeon.hero != null && Dungeon.hero.heroClass == HeroClass.DUELIST && hasDuelistAbility()){
			info += "\n\n" + abilityInfo();
		}

		if (levelKnown && seed != null){
			info += "\n\n" + Messages.get(MeleeWeapon.class, "poison_desc", seed.name(), poisonTurns);
		}
	...
	protected static WndBag.ItemSelector seedSelector = new WndBag.ItemSelector() {

		@Override
		public String textPrompt() {
			return Messages.get(MeleeWeapon.class, "inv_title");
		}
		...
		public void onSelect( Item item ) {
			if (item instanceof Plant.Seed && curItem instanceof MeleeWeapon) {
				MeleeWeapon weapon = (MeleeWeapon) curItem;
				weapon.seed = (Plant.Seed) Reflection.newInstance(item.getClass()).quantity(1);
				weapon.setPoisonTurns(5);
				item.detach(curUser.belongings.backpack);
				curUser.spend( TIME_TO_POISON );
				curUser.busy();
				curUser.sprite.operate(curUser.pos);
				GLog.i(Messages.get(MeleeWeapon.class, "poisoned", weapon.name(), item.name()));
				updateQuickslot();
			}
		}
	};
	...
	public void setPoisonTurns( int turns ){
		this.poisonTurns = turns + this.level();
	}

	@Override
	public int proc(Char attacker, Char defender, int damage) {
		if (seed != null && poisonTurns > 0){
			seed.onProc(attacker, defender, damage);
			poisonTurns--;
			if (poisonTurns <= 0){
				seed = null;
				GLog.i(Messages.get(MeleeWeapon.class, "wears_off", name()));
			}
		}
	...
	public Emitter emitter() {
		if (seed == null || seed.getPixelParticle() == null) {
			return null;
		}
	...
	public void storeInBundle( Bundle bundle ) {
		...
		bundle.put( POISON_TURNS, poisonTurns );
	}
	...
	public void restoreFromBundle( Bundle bundle ) {
		...
		poisonTurns = bundle.getInt( POISON_TURNS );
	}
