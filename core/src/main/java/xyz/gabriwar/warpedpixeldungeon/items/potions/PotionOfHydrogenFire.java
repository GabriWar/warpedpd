...
public class PotionOfHydrogenFire extends Potion {
    ...
    {
        icon = ItemSpriteSheet.Icons.POTION_HYDROFIRE;
    }
    ...
    public void apply(Hero hero) {
        ...
        for (int offset : PathFinder.NEIGHBOURS9) {
            if (!Dungeon.level.solid[hero.pos + offset]) {
                GameScene.add(Blob.seed(hero.pos + offset, 2, HalomethaneFire.class));
            }
        }
    }
    ...
    public void shatter(int cell) {
        ...
        for (int offset : PathFinder.NEIGHBOURS9) {
            ...
            if (!Dungeon.level.solid[c]) {
                GameScene.add(Blob.seed(c, 2, HalomethaneFire.class));
            }
