...
import xyz.gabriwar.warpedpixeldungeon.sprites.TenguSprite;
...
public class Tengu extends Mob {
	
	{
		...
		declareExtraLoot(TengusMask.class, 1f);
	}
	
	//The den boss (depth 36) is the same class, but a far heavier fight: Sprouted's TenguDen
	//had double the HP and defense, flat DR, double speed and far harder hits. The merge kept
	//the arena Tengu's numbers, so the den boss came out half-strength. Flagged rather than
	//tested with instanceof, because the initializer runs while Dungeon.level is still the
	//level being left.
	public boolean denBoss = false;

	public void makeDenBoss() {
		denBoss = true;
	...
	public int damageRoll() {
		return denBoss ? Random.NormalIntRange( 15, 25 )
	...
	@Override
	public int drRoll() {
		return denBoss ? super.drRoll() + 20
		               : super.drRoll() + Random.NormalIntRange(0, 5);
	}
	...
	public void damage(int dmg, Object src) {
		...
		//merged den boss: outside the prison arena (the Tengu Den, depth 36) there
		//is no arena state machine, so take damage as an ordinary boss and die
		//normally. Leaves the prison arena path below completely untouched.
	...
	public void die( Object cause ) {
		...
		//merged den boss: the depth-36 Tengu Den sets its own flag (which unlocks
		//the TenguKey) and does not credit the prison Tengu's badges/score/beacon.
		if (Dungeon.level instanceof TenguDenLevel) {
	...
	protected void dropExtraLoot() {
		//den boss: a gold hoard plus the guaranteed AdamantRing. No prison rewards.
		if (Dungeon.level instanceof TenguDenLevel) {
			Dungeon.level.drop(new Gold(Random.NormalIntRange(1900, 4000)), pos).sprite.drop();
			//the den's guaranteed ring. Without it the run gets one instead of two, and the
			//Blacksmith consumes one of them.
			Dungeon.level.drop(new AdamantRing(), pos).sprite.drop();
			return;
		}
		if (Dungeon.hero.subClass == HeroSubClass.NONE) {
			trackedDrop(new TengusMask(), 0);
		}
		//Sprouted parity: the prison Tengu drops the Sokoban2 journal page (not subclass-gated)
		trackedDrop(new xyz.gabriwar.warpedpixeldungeon.items.journalpages.Sokoban2(), 1);
	}
	...
	private static final String DEN_BOSS = "den_boss";
