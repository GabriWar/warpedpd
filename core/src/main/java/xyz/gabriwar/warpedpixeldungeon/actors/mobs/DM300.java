...
public class DM300 extends Mob {

	{
		...
		declareExtraLoot(MetalShard.class, 1f);
	}
	...
	protected void dropExtraLoot() {
		...
		for (int i = 0; i < shards; i++){
			...
			if (firstShard) {
				trackedDrop(new MetalShard(), pos + ofs, 0);
				firstShard = false;
			} else {
				Dungeon.level.drop( new MetalShard(), pos + ofs ).sprite.drop( pos );
			}
		}
		trackedDrop(new xyz.gabriwar.warpedpixeldungeon.items.journalpages.Sokoban3(), 1);
		if (!Dungeon.LimitedDrops.JOURNAL.dropped()) {
			trackedDrop(new xyz.gabriwar.warpedpixeldungeon.items.OtilukesJournal(), 2);
