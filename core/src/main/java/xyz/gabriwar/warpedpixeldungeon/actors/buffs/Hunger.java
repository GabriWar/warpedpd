...
public class Hunger extends Buff implements Hero.Doom {
	...
	public boolean act() {
		...
		if (target.isAlive() && target instanceof Hero) {
			...
			//an exhausted or starving mind starts seeing things
			xyz.gabriwar.warpedpixeldungeon.actors.mobs.Mirage.trySpawn( hero );
			...
			if (isStarving()) {
				...
				// sleeping or resting burns food much slower
				if (target.buff(MagicalSleep.class) != null
						|| (target instanceof Hero && ((Hero)target).resting)){
					hungerDelay *= 2f;
				}
				// Heatstroke makes hunger accumulate 20% faster
				Heatstroke heat = target.buff(Heatstroke.class);
				if (heat != null) {
					hungerDelay /= heat.hungerFactor();
				}
	...
	// MP: host pushes its hunger level to clients so they don't drift / starve solo.
