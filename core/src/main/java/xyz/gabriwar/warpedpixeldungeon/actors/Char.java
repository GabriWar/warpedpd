...
import xyz.gabriwar.warpedpixeldungeon.actors.hero.spells.BeamingRay;
...
import xyz.gabriwar.warpedpixeldungeon.items.weapon.missiles.darts.ShockingDart;
...
public abstract class Char extends Actor {
	...
	public float bodyTemp = Float.NaN;
	...
	public boolean interact(Char c){
		...
		if (paralysed > 0 || c.paralysed > 0 || rooted || c.rooted
				...
				|| buff(Drunk.class) != null || c.buff(Drunk.class) != null){
			return true;
		}
	...
	protected static final String BODY_TEMP = "bodyTemp";
	...
	public void storeInBundle( Bundle bundle ) {
		...
		if (!Float.isNaN(bodyTemp)) bundle.put( BODY_TEMP, bodyTemp );
	}
	...
	public void restoreFromBundle( Bundle bundle ) {
		...
		if (bundle.contains( BODY_TEMP )) bodyTemp = bundle.getFloat( BODY_TEMP );
	}
	...
	public boolean attack( Char enemy, float dmgMulti, float dmgBonus, float accMulti ) {
		...
		if (enemy.isInvulnerable(getClass())) {
			...
			if (this == Dungeon.hero && Dungeon.debugOneHitKill){
				enemy.damage( 1_000_000, this );
			...
			// Undead mobs deal more damage at night
			if (properties().contains(Property.UNDEAD)) {
				dmg *= DayNightCycle.undeadDamageMultiplier();
			}

			// Season and weekday damage modifiers
			dmg *= GameCalendar.seasonDamageMultiplier();
			dmg *= GameCalendar.weekdayDamageMultiplier();
	...
	public static boolean hit( Char attacker, Char defender, float accMulti, boolean magic ) {
		...
		if (attacker == Dungeon.hero && Dungeon.debugOneHitKill) return true;
	...
	public int drRoll() {
		...
		// Weekday defense bonus (hero only)
		if (this == Dungeon.hero) {
			dr = Math.round(dr * GameCalendar.weekdayDefenseMultiplier());
		}
	...
	//the empowerment power() of a matching glyph, for scaling glyph effects
	//computed outside of proc(). mirrors glyphLevel's resolution chain
	public float glyphPower(Class<? extends Armor.Glyph> cls){
		if (Dungeon.hero != null && Dungeon.level != null
				&& this != Dungeon.hero && Dungeon.hero.alignment == alignment
				&& Dungeon.hero.buff(AuraOfProtection.AuraBuff.class) != null
				&& (Dungeon.level.distance(pos, Dungeon.hero.pos) <= 2 || buff(LifeLinkSpell.LifeLinkSpellBuff.class) != null)) {
			return Dungeon.hero.glyphPower(cls);
		} else {
			return 1f;
		}
	}
	...
	public void damage( int dmg, Object src ) {
		...
		if (buff(RoseBarrier.class) != null || buff(Cocoshield.class) != null) {
			sprite.showStatus(CharSprite.POSITIVE, Messages.get(this, "invulnerable"));
			return;
		}
		...
		if (buff(IronSkin.class) != null && !isImmune(IronSkin.class)) {
			dmg = 1;
		}

		//Shield (PotionOfArmor): soaks a fraction of each hit for a limited number
		//of hits. Its detach() decrements the hit counter and only removes the buff
		//once spent. Nothing used to call it, so the potion did nothing at all.
		Shield potionShield = buff(Shield.class);
		if (potionShield != null && dmg > 0) {
			dmg = Math.round(dmg * (1f - Shield.LEVEL));
			potionShield.detach();
		}
	...
	protected void spend( float time ) {
		...
		if (buff( xyz.gabriwar.warpedpixeldungeon.actors.buffs.Hypothermia.class ) != null) {
			timeScale *= buff( xyz.gabriwar.warpedpixeldungeon.actors.buffs.Hypothermia.class ).speedFactor();
		}
	...
	public float stealth() {
		...
		if (buff(xyz.gabriwar.warpedpixeldungeon.actors.buffs.Crouching.class) != null) {
			stealth += xyz.gabriwar.warpedpixeldungeon.actors.buffs.Crouching.STEALTH_BONUS;
		}

		// Night provides stealth bonus to the hero
		if (this == Dungeon.hero) {
			stealth += DayNightCycle.stealthBonus();
			stealth += GameCalendar.weekdayEvasionBonus();
			//skill tree: Rogue's Stealth
			stealth += ((xyz.gabriwar.warpedpixeldungeon.actors.hero.Hero) this).heroSkills.allStealth();
		}

		// Soaked shoes make squelching sounds — harder to stay hidden
		if (buff(SoakedShoes.class) != null) {
			stealth += SoakedShoes.STEALTH_PENALTY;
		}
	...
	public void move( int step, boolean travelling ) {
		...
		if (travelling && Dungeon.level.adjacent( step, pos ) && (buff( Vertigo.class ) != null || buff( Drunk.class ) != null)) {
		...
		if (travelling) {
			...
			if (buff(TrailOfFire.class) != null && !isImmune(TrailOfFire.class)) {
				GameScene.add(Blob.seed(prevPos, 1, Fire.class));
			}
			// Sprouting: leave high grass on the previous tile
			if (buff(Sprouting.class) != null && !isImmune(Sprouting.class)
					&& (Dungeon.level.map[prevPos] == Terrain.GRASS || Dungeon.level.map[prevPos] == Terrain.EMPTY)) {
				Level.set(prevPos, Terrain.HIGH_GRASS);
				GameScene.updateMap(prevPos);
			}
			// HeatAura: dry adjacent water tiles
			if (buff(HeatAura.class) != null) {
				for (int i : PathFinder.NEIGHBOURS9) {
					int cell = pos + i;
					if (cell >= 0 && cell < Dungeon.level.length() && Dungeon.level.map[cell] == Terrain.WATER) {
						Dungeon.level.map[cell] = Terrain.EMPTY;
						GameScene.updateMap(cell);
					}
				}
			}
			// IceAura: apply Frost to all chars in field of view
			if (buff(IceAura.class) != null && fieldOfView != null) {
				for (Char ch : chars()) {
					if (ch != this && fieldOfView.length > ch.pos && fieldOfView[ch.pos]) {
						Buff.prolong(ch, Frost.class, Frost.DURATION * Random.Float(1f, 1.5f));
					}
				}
			}
			// SuperBalling: spread Vertigo to all chars in field of view
			if (buff(SuperBalling.class) != null && fieldOfView != null) {
				for (Char ch : chars()) {
					if (ch != this && fieldOfView.length > ch.pos && fieldOfView[ch.pos]) {
			...
			// Slippery: chance to fall into adjacent chasm when moving
			if (buff(Slippery.class) != null && this instanceof Hero && !flying) {
				for (int i : PathFinder.NEIGHBOURS8) {
					int cell = pos + i;
					if (cell >= 0 && cell < Dungeon.level.length() && Dungeon.level.map[cell] == Terrain.CHASM) {
	...
	//Re-ARranged PD helper: heal with cap + floating text
