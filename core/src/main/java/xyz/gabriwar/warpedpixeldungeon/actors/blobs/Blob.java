...
public class Blob extends Actor {
	...
	public int volume = 0;
	/**
	 * Shifts this blob's contents by (-dx, -dy) cells inside a window of the
	 * given size - used when a sliding-window level rebases. Cells that fall
	 * outside the window are dropped and the volume is recomputed. Without
	 * this, a rebased level leaves the blob anchored to WINDOW coordinates:
	 * fire keeps burning at the same screen position over brand-new terrain.
	 */
	public void translate( int dx, int dy, int width, int height ){
		if (cur == null) return;
		int[] newCur = new int[cur.length];
		int newVolume = 0;
		//the outer ring is excluded: the engine's evolve() overrides iterate
		//area.left-1..right / top-1..bottom with no bounds guard, so content
		//on the ring would make them index outside the map and crash
		...
		if (newVolume > 0){
			for (int i = 0; i < cur.length; i++){
				if (cur[i] > 0) area.union( i % width, i / width );
			}
		}
	}
