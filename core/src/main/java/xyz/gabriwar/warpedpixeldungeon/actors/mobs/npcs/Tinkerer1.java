...
import xyz.gabriwar.warpedpixeldungeon.windows.WndTinkerer;
...
public class Tinkerer1 extends NPC {
	...
	public boolean interact( Char c ) {
		...
		if (item != null && vial != null) {
			Game.runOnRenderThread(new Callback() {
				@Override
				public void call() {
					GameScene.show( new WndTinkerer( Tinkerer1.this, item ) );
				}
			});
	...
	private void tell( String text ) {
		Game.runOnRenderThread(new Callback() {
			@Override
			public void call() {
				GameScene.show( new WndQuest( Tinkerer1.this, text ) );
			}
		});
	}
}
