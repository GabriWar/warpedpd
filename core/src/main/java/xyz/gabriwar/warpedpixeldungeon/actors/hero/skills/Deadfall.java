...
public class Deadfall extends Skill {
	...
	public void execute( Hero hero, String action ){
		if (action.equals(Skill.AC_CAST) && level > 0 && hero.MP >= getManaCost()){
			GameScene.selectCell( new Dropper() );
			Dungeon.hero.heroSkills.lastUsed = this;
		}
	}

	private class Dropper extends CellSelector.Listener {
		...
		public void onSelect( Integer target ){
			...
			int cell = target;
			if (cell < 0 || cell >= Dungeon.level.length() || !Dungeon.level.heroFOV[cell]){
			...
			for (int n : PathFinder.NEIGHBOURS9){
				...
				if (ch != null && ch != curUser && ch.alignment == Char.Alignment.ENEMY){
					ch.damage( Random.NormalIntRange( 4 + 3 * level, 8 + 6 * level ), curUser );
					if (ch.isAlive() && !ch.properties().contains( Char.Property.BOSS )){
						Buff.affect( ch, Paralysis.class, level >= 3 ? 2f : 1f );
					}
				}
			}
		...
		public String prompt(){
			return "Choose where to drop the weight";
		}
	}
