...
public class Freezing extends Blob {
		...
		for (int i = area.left-1; i <= area.right; i++) {
			for (int j = area.top-1; j <= area.bottom; j++) {
				...
				if (Dungeon.level.heroFOV[cell]) {
					CellEmitter.get(cell).burst(SnowParticle.RISING_FACTORY, Dungeon.level.water[cell] ? 4 : 2);
				}
	...
	public static void freeze( int cell ){
		...
		if (ch != null && !ch.isImmune(Freezing.class)) {
			if (ch.buff(Frost.class) != null){
				...
				if (chill != null
					...
					//big burst of rising snow when a character fully freezes
					if (Dungeon.level.heroFOV[cell]) {
						CellEmitter.get(cell).burst(SnowParticle.RISING_FACTORY, 8);
						CellEmitter.center(cell).burst(Speck.factory(Speck.BLIZZARD), 3);
					}
				}
			}
		}
		...
		if (heap != null) {
			heap.freeze();
			if (Dungeon.level.heroFOV[cell]) {
				CellEmitter.get(cell).burst(SnowParticle.FACTORY, 4);
			}
		}
	}
