...
public abstract class Actor implements Bundlable {
		...
		do {
			...
			if  (current != null) {
				...
				if (xyz.gabriwar.warpedpixeldungeon.net.NetManager.isHost()) {
					...
					int pos = acting instanceof Char ? ((Char) acting).pos : -1;
					xyz.gabriwar.warpedpixeldungeon.net.NetManager.log("[NET-HOST] Actor.pick id=" + acting.id() + " " + tag
							+ " pos=" + pos + " time=" + acting.time);
				}
					...
					try {
						synchronized (((Char)acting).sprite) {
							if (((Char)acting).sprite.isMoving) {
								...
								if (xyz.gabriwar.warpedpixeldungeon.net.NetManager.isHost()) {
									xyz.gabriwar.warpedpixeldungeon.net.NetManager.log("[NET-HOST] Actor sprite-wait done id="
											+ acting.id());
								}
							}
						}
			...
			if (!doNext){
				synchronized (Thread.currentThread()) {
					...
					//broadcast state to spectators when the game pauses for hero input
					if (xyz.gabriwar.warpedpixeldungeon.net.NetManager.isHost()) {
						xyz.gabriwar.warpedpixeldungeon.net.NetManager.broadcastGameState();
					}
	...
	public static synchronized Char findChar( int pos ) {
		for (Char ch : chars){
			if (ch.pos == pos) {
				// Net MP: heroes parked at an exit are functionally off-map —
				// invisible, untargetable, no pathfinding collision. Pretend
				// they aren't there so other actors can step onto the stairs.
				if (ch instanceof xyz.gabriwar.warpedpixeldungeon.actors.hero.Hero
						&& ((xyz.gabriwar.warpedpixeldungeon.actors.hero.Hero) ch).atExit) {
					continue;
				}
				return ch;
			}
