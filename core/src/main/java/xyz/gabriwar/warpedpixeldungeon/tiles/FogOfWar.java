...
public class FogOfWar extends Image {
	...
	*/
	...
	private static Pixmap shiftScratch;

	/**
	 * Sliding-window rebase fast path: blits the fog texture by the shift so
	 * the overlap keeps its already-rendered fog, and marks only the exposed
	 * strips for re-rendering. The full-map fog rebuild was a major part of
	 * the rebase's heavy frame.
	 */
	public synchronized void shiftContent( int dcx, int dcy ){
		Pixmap fog = texture.bitmap;
		if (shiftScratch == null
				|| shiftScratch.getWidth() != fog.getWidth()
				|| shiftScratch.getHeight() != fog.getHeight()){
			if (shiftScratch != null) shiftScratch.dispose();
			shiftScratch = new Pixmap( fog.getWidth(), fog.getHeight(), fog.getFormat() );
		}
		shiftScratch.setBlending( Pixmap.Blending.None );
		...
		//previous window, which painted ghost 'explored' blotches. blacken
		//the strips first so that assumption holds again
		fog.setColor( 0x000000FF );
		if (dcx < 0){
			fog.fillRectangle( 0, 0, (-dcx + fringe) * PIX_PER_TILE, mapHeight * PIX_PER_TILE );
			toUpdate.add( new Rect( 0, 0, -dcx + fringe, mapHeight ) );
		} else if (dcx > 0){
			fog.fillRectangle( (mapWidth - dcx - fringe) * PIX_PER_TILE, 0,
					(dcx + fringe) * PIX_PER_TILE, mapHeight * PIX_PER_TILE );
			toUpdate.add( new Rect( mapWidth - dcx - fringe, 0, mapWidth, mapHeight ) );
		}
		if (dcy < 0){
			fog.fillRectangle( 0, 0, mapWidth * PIX_PER_TILE, (-dcy + fringe) * PIX_PER_TILE );
			toUpdate.add( new Rect( 0, 0, mapWidth, -dcy + fringe ) );
		} else if (dcy > 0){
			fog.fillRectangle( 0, (mapHeight - dcy - fringe) * PIX_PER_TILE,
					mapWidth * PIX_PER_TILE, (dcy + fringe) * PIX_PER_TILE );
			toUpdate.add( new Rect( 0, mapHeight - dcy - fringe, mapWidth, mapHeight ) );
		}
		//borders are window-authored - keep them exact
		toUpdate.add( new Rect( 0, 0, mapWidth, 1 ) );
		toUpdate.add( new Rect( 0, mapHeight-1, mapWidth, mapHeight ) );
		toUpdate.add( new Rect( 0, 0, 1, mapHeight ) );
	...
	private void updateTexture( boolean[] visible, boolean[] visited, boolean[] mapped ) {
		...
		this.mapped = mapped;
		this.brightness = WPDSettings.brightness() + 1;
		...
		for (Rect update : updating) {
			for (int i = update.top; i < update.bottom; i++) {
				...
				for (int j = update.left; j < update.right; j++) {
					...
					//a level (or cell) with no fog at all: skip the wall-darkening rules too, which
					//black out wall clusters (tree stands, building innards) regardless of FOV
					if (Dungeon.level.plainFogAt(cell)) {
						fillCell(fog, j, i, FOG_COLORS[VISIBLE][brightness]);
						cell++;
						continue;
					}
