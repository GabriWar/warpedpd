...
public abstract class CustomTilemap implements Bundlable {
	...
	//the shape the tilemap on screen was last filled with, so a streaming
	//level can refill it instead of building a new one
	private int visCols = -1, visLen = -1;

	/**
	 * Builds - or, when one is already on screen with the same shape, refills -
	 * this layer's tilemap. A streaming level rebuilds its layers every time its
	 * window moves, and every fresh Tilemap means a fresh vertex buffer: ~2MB of
	 * direct memory for a 176x176 window, times a layer count, times every slide.
	 */
	public Tilemap create( int[] data, int cols ){
		...
		//NOT create(): subclasses override it to route back here, and a virtual
		//call would recurse until the stack gives out
		Tilemap v = build();
		v.map( data, cols );
		visCols = cols;
		visLen = data.length;
	...
	private Tilemap build(){
		//killAndErase drops it from the scene but never frees its vertex buffer:
		//Vertexbuffer holds every buffer it ever made in a static list, so an
		//undestroyed tilemap leaks its direct memory for the rest of the process
		if (vis != null){
