...
import xyz.gabriwar.warpedpixeldungeon.actors.mobs.npcs.ImpShopkeeper;
...
import xyz.gabriwar.warpedpixeldungeon.sprites.SpawnerSprite;
	...
	public static class LandmarkRecord extends Record {
		...
		public Image icon(){
			switch (landmark){
				...
				case ALTAR_SHRINE:
					return Icons.get(Icons.ALTAR_SHRINE);
				case STATUE:
				...
				case PORTAL: {
					Image img = new Image(xyz.gabriwar.warpedpixeldungeon.Assets.Interfaces.PORTAL_ICON);
					xyz.gabriwar.warpedpixeldungeon.actors.mobs.npcs.PortalGate.State s
							= xyz.gabriwar.warpedpixeldungeon.Portals.getState(depth);
					switch (s) {
						case ACTIVE: img.hardlight(0.6f, 0.85f, 1f); break;       // blue
						case DEAD:   img.hardlight(0.45f, 0.45f, 0.45f); break;   // grey
						case LOCKED:
			...
			switch (landmark) {
				case PORTAL: {
					xyz.gabriwar.warpedpixeldungeon.actors.mobs.npcs.PortalGate.State s
							= xyz.gabriwar.warpedpixeldungeon.Portals.getState(depth);
					switch (s) {
						case ACTIVE: return "portal gate (active)";
						case DEAD:   return "portal gate (broken)";
						case LOCKED:
						default:     return "portal gate (locked)";
					}
				}
		...
		public String desc() {
			switch (landmark) {
				...
				case ALTAR_SHRINE:      return Messages.get(AltarShrine.class, "desc");
				...
				case PORTAL: {
					xyz.gabriwar.warpedpixeldungeon.actors.mobs.npcs.PortalGate.State s
							= xyz.gabriwar.warpedpixeldungeon.Portals.getState(depth);
					switch (s) {
						case ACTIVE: return "An active portal gate. You can travel here from any other active gate.";
						case DEAD:   return "A portal gate whose keeper has been killed. It no longer functions and cannot be reached.";
						case LOCKED:
						default:     return "A dormant portal gate. Pay its keeper to activate it for fast travel.";
					}
				}
			}
		}
