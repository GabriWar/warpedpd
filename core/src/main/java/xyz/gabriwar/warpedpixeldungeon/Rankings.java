...
public enum Rankings {
	...
	public void submit( boolean win, Object cause ) {
		...
		Matcher m = p.matcher(WarpedPixelDungeon.version);
		if (m.find()) {
		...
		rec.heroName    = Dungeon.hero.name();
	...
	public int calculateScore(){
		...
		if (Dungeon.initialVersion > WarpedPixelDungeon.v1_2_3){
	...
	public void load() {
		...
		try {
			...
			if (bundle.contains(LATEST_DAILY)){
				...
				if (latestDate > WPDSettings.lastDaily()){
					WPDSettings.lastDaily(latestDate);
				}
			}
	...
	public static class Record implements Bundlable {
		...
		public void restoreFromBundle( Bundle bundle ) {
			...
			heroName    = bundle.contains(HERO_NAME) ? bundle.getString(HERO_NAME) : null;
		...
		public void storeInBundle( Bundle bundle ) {
			...
			if (heroName != null) bundle.put( HERO_NAME, heroName );
