...
import xyz.gabriwar.warpedpixeldungeon.debug.SpriteInspector;
...
import xyz.gabriwar.warpedpixeldungeon.ui.Tooltip;
...
public class PixelScene extends Scene {
	...
	public void create() {
		...
		Game.platform.setupFontGenerators(renderedTextPageSize, WPDSettings.systemFont());
	...
	public void update() {
		...
		if (DeviceCompat.isDesktop() && fullscreenListener == null){
			KeyEvent.addKeyListener(fullscreenListener = new Signal.Listener<KeyEvent>() {
				...
				public boolean onSignal(KeyEvent keyEvent) {
					...
					if (alt && enter){
						WPDSettings.fullscreen(!WPDSettings.fullscreen());
						return true;
					}
		...
		if (!Cursor.isCursorCaptured()) {
			if (Math.abs(ControllerHandler.rightStickPosition.x) >= 0.2f
				...
				int sensitivity = WPDSettings.controllerPointerSensitivity() * 100;
	...
	public static boolean landscape(){
		return WPDSettings.interfaceSize() > 0 || Game.width > Game.height;
	}
	...
	public static void shake( float magnitude, float duration){
		magnitude *= WPDSettings.screenShake();
