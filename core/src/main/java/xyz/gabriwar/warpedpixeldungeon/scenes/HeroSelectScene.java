...
public class HeroSelectScene extends PixelScene {
	...
	public static String pendingCustomName = null;
	...
	public void create() {
		...
		infoButton = new IconButton(Icons.get(Icons.INFO)){
			...
			protected void onClick() {
				...
				if (cls != null) {
					...
					WarpedPixelDungeon.scene().addToFront(info);
				}
			}
		...
		renameButton = new IconButton(Icons.get(Icons.SCROLL_GREY)){
			@Override
			protected void onClick() {
				super.onClick();
				WarpedPixelDungeon.scene().addToFront(new WndTextInput(
						Messages.get(HeroSelectScene.class, "rename_title"),
						Messages.get(HeroSelectScene.class, "rename_body"),
						pendingCustomName != null ? pendingCustomName : "",
						20,
						false,
						Messages.get(HeroSelectScene.class, "rename_confirm"),
						Messages.get(HeroSelectScene.class, "rename_cancel")){
					@Override
					public void onSelect(boolean positive, String text) {
						if (positive) {
							pendingCustomName = text.trim().isEmpty() ? null : text.trim();
							if (GamesInProgress.selectedClass != null) {
			...
			@Override
			protected String hoverText() {
				return Messages.get(HeroSelectScene.class, "rename_title");
			}
		};
		renameButton.visible = renameButton.active = false;
		renameButton.setSize(20, 21);
		add(renameButton);
		...
		if (!Badges.isUnlocked(Badges.Badge.VICTORY) && !DeviceCompat.isDebug()){
			...
			WPDSettings.challenges(0);
			WPDSettings.customSeed("");
		}
		...
		add( btnExit );
		btnExit.visible = btnExit.active = !WPDSettings.intro();
		...
		if (Badges.isUnlocked(Badges.Badge.VICTORY) && !WPDSettings.victoryNagged()) {
			WPDSettings.victoryNagged(true);
	...
	private void updateOptionsColor(){
		if (!WPDSettings.customSeed().isEmpty()){
			btnOptions.icon().hardlight(1f, 1.5f, 0.67f);
		} else if (WPDSettings.challenges() != 0){
	...
	private void setSelectedHero(HeroClass cl){
		...
		if (landscape()) {
			...
			heroName.text(pendingCustomName != null ? pendingCustomName : Messages.titleCase(cl.title()));
			...
			renameButton.visible = renameButton.active = true;
			renameButton.setPos(infoButton.right(), heroName.top() + (heroName.height() - renameButton.height())/2f);
			align(renameButton);

			btnOptions.visible = btnOptions.active = !WPDSettings.intro();

		} else {
			title.visible = false;

			startBtn.visible = startBtn.active = true;
			startBtn.text(pendingCustomName != null ? pendingCustomName : Messages.titleCase(cl.title()));
			startBtn.setSize(startBtn.reqWidth() + 8, 21);
			...
			renameButton.visible = renameButton.active = true;
			renameButton.setPos(infoButton.right(), startBtn.top());
	...
	public void update() {
		super.update();
		if (WPDSettings.intro() && Rankings.INSTANCE.totalNumber > 0){
	...
	private class HeroBtn extends StyledButton {
		...
		protected void onClick() {
			...
			if( !cl.isUnlocked() ){
				WarpedPixelDungeon.scene().addToFront( new WndMessage(cl.unlockMsg()));
			} else if (GamesInProgress.selectedClass == cl) {
				Window w = new WndHeroInfo(cl);
				if (landscape()){
					w.offset(Camera.main.width/6, 0);
				}
				WarpedPixelDungeon.scene().addToFront(w);
			} else {
				setSelectedHero(cl);
			}
		}
	...
	private class GameOptions extends Component {
		...
		protected void createChildren() {
			...
			StyledButton seedButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "custom_seed"), 6){
				...
				protected void onClick() {
					if (!Badges.isUnlocked(Badges.Badge.VICTORY) && !DeviceCompat.isDebug()){
						WarpedPixelDungeon.scene().addToFront( new WndTitledMessage(
					...
					String existingSeedtext = WPDSettings.customSeed();
					WarpedPixelDungeon.scene().addToFront( new WndTextInput(Messages.get(HeroSelectScene.class, "custom_seed_title"),
							...
							if (positive && seed != -1){
								...
								for (GamesInProgress.Info info : GamesInProgress.checkAll()){
									if (info.customSeed.isEmpty() && info.seed == seed){
										WPDSettings.customSeed("");
										icon.resetColor();
										WarpedPixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "custom_seed_duplicate")));
										return;
									}
								}

								WPDSettings.customSeed(text);
			...
			if (!WPDSettings.customSeed().isEmpty()) seedButton.icon().hardlight(1f, 1.5f, 0.67f);;
			...
			StyledButton dailyButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "daily"), 6){
				...
				protected void onClick() {
					...
					long diff = (WPDSettings.lastDaily() + DAY) - Game.realTime;
					if (diff > 24*HOUR){
						WarpedPixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_unavailable_long", (diff / DAY)+1)));
						return;
					}

					for (GamesInProgress.Info game : GamesInProgress.checkAll()){
						if (game.daily){
							WarpedPixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_existing")));
							return;
						}
					}
			...
			challengeButton = new StyledButton(Chrome.Type.BLANK, Messages.get(WndChallenges.class, "title"), 6){
				...
				protected void onClick() {
					...
					WarpedPixelDungeon.scene().addToFront(new WndChallenges(WPDSettings.challenges(), true) {
						public void onBackPressed() {
							super.onBackPressed();
							icon(Icons.get(WPDSettings.challenges() > 0 ? Icons.CHALLENGE_COLOR : Icons.CHALLENGE_GREY));
							updateOptionsColor();
						}
					} );
				}
			...
			challengeButton.icon(Icons.get(WPDSettings.challenges() > 0 ? Icons.CHALLENGE_COLOR : Icons.CHALLENGE_GREY));
			add(challengeButton);
			...
			if (unlockedCount >= 2) {
				StyledButton randomButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "randomize"), 6) {
					...
					protected void onClick() {
						...
						if (Badges.isUnlocked(Badges.Badge.VICTORY) || DeviceCompat.isDebug()){
							WarpedPixelDungeon.scene().addToFront(new WndRandomize());
						} else {
		...
		private class WndRandomize extends Window {
			...
			public WndRandomize(){
				...
				optChals.setSelectedValue(Challenges.activeChallenges(WPDSettings.challenges()));
				...
				RedButton btnConfirm = new RedButton(Messages.get(HeroSelectScene.class, "randomize_confirm")){
					...
					protected void onClick() {
						...
						if (chkChals.checked()){
							...
							WPDSettings.challenges(mask);
							challengeButton.icon(Icons.get(WPDSettings.challenges() > 0 ? Icons.CHALLENGE_COLOR : Icons.CHALLENGE_GREY));
							WarpedPixelDungeon.scene().addToFront(new WndChallenges(mask, false));
						}
