...
import xyz.gabriwar.warpedpixeldungeon.effects.WobblyBanner;
import xyz.gabriwar.warpedpixeldungeon.effects.Fireball;
...
public class TitleScene extends PixelScene {
	...
	private WobblyBanner title;
	private Fireball leftFB;
	private Fireball rightFB;
	private WobblyBanner signs;
	...
	public void create() {
		...
		//the wordmark wobbles: sliced over a travelling wave, catching the light as it tips
		title = new WobblyBanner(
				landscape() ? BannerSprites.Type.TITLE_LAND : BannerSprites.Type.TITLE_PORT,
				1.6f, 60f, 2.6f, false );
		add( title );
		...
		title.x = align(title.x);
		title.y = align(title.y);
		...
		signs = new WobblyBanner(
				landscape() ? BannerSprites.Type.TITLE_GLOW_LAND : BannerSprites.Type.TITLE_GLOW_PORT,
				1.6f, 60f, 2.6f, true ){
		...
		btnPlay = new StyledButton(GREY_TR, Messages.get(this, "enter")){
			...
			protected void onClick() {
				if (GamesInProgress.checkAll().size() == 0){
					...
					WarpedPixelDungeon.switchNoFade( StartScene.class );
				}
			}
		...
		btnRankings = new StyledButton(GREY_TR,Messages.get(this, "rankings")){
			@Override
			protected void onClick() {
				WarpedPixelDungeon.switchNoFade( RankingsScene.class );
			}
		};
		...
		btnAbout = new StyledButton(GREY_TR, Messages.get(this, "about")){
			@Override
			protected void onClick() {
				WarpedPixelDungeon.switchScene( AboutScene.class );
			}
		};
		btnAbout.icon(Icons.get(Icons.WPD));
		add(btnAbout);

		btnMultiplayer = new StyledButton(GREY_TR, Messages.get(this, "multiplayer")){
			@Override
			protected void onClick() {
				TitleScene.this.add(new xyz.gabriwar.warpedpixeldungeon.net.ui.WndMultiplayer());
			}
		};
		btnMultiplayer.icon(Icons.get(Icons.CONTROLLER));
		add(btnMultiplayer);
		...
		if (landscape()) {
			...
			btnSupport.setRect(btnPlay.left(), btnPlay.bottom()+ GAP, buttonAreaWidth, BTN_HEIGHT);
			btnRankings.setRect(btnPlay.left(), btnSupport.bottom()+ GAP, (float) (Math.floor(buttonAreaWidth/3f)-1), BTN_HEIGHT);
			...
			float qw = (float)(Math.floor(buttonAreaWidth/3f)-1);
			btnSettings.setRect(btnRankings.left(), btnRankings.bottom() + GAP, qw, BTN_HEIGHT);
			btnChanges.setRect(btnSettings.right()+2, btnSettings.top(), qw, BTN_HEIGHT);
			btnAbout.setRect(btnChanges.right()+2, btnSettings.top(), qw, BTN_HEIGHT);
		} else {
			...
			btnSupport.setRect(btnPlay.left(), btnMultiplayer.bottom()+ GAP, btnPlay.width(), BTN_HEIGHT);
	...
	private static class NewsButton extends StyledButton {
		...
		public NewsButton(Chrome.Type type, String label ){
			super(type, label);
			if (WPDSettings.news()) News.checkForNews();
		}
			...
			if (unreadCount == -1 && News.articlesAvailable()){
				...
				if (lastRead == 0){
					if (News.articles().get(0) != null) {
						WPDSettings.newsLastRead(News.articles().get(0).date.getTime());
					}
				} else {
					unreadCount = News.unreadArticles(new Date(WPDSettings.newsLastRead()));
					if (unreadCount > 0) {
		...
		protected void onClick() {
			super.onClick();
			WarpedPixelDungeon.switchNoFade( NewsScene.class );
		}
	}
	...
	private static class ChangesButton extends StyledButton {
		...
		public ChangesButton( Chrome.Type type, String label ){
			super(type, label);
			if (WPDSettings.updates()) Updates.checkForUpdate();
		}
	...
	private static class SettingsButton extends StyledButton {
		...
		protected void onClick() {
			...
			WarpedPixelDungeon.scene().add(new WndSettings());
		}
	}
	...
	private static class SupportButton extends StyledButton{
		...
		@Override
		protected void onClick() {
			WarpedPixelDungeon.switchNoFade(SupporterScene.class);
		}
	}
}
