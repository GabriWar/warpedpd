...
public class StartScene extends PixelScene {
	...
	public void create() {
		...
		String sortText = "";
		switch (WPDSettings.gamesInProgressSort()){
			case "level":
		...
		StyledButton btnSort = new StyledButton(Chrome.Type.TOAST_TR, sortText, 6){
			...
			protected void onClick() {
				...
				if (WPDSettings.gamesInProgressSort().equals("level")){
					WPDSettings.gamesInProgressSort("last_played");
				} else {
					WPDSettings.gamesInProgressSort("level");
				}
		...
		public void set( int slot ){
			...
			if (newGame){
				...
				if (info.heroName != null && !info.heroName.isEmpty()) {
					name.text(Messages.titleCase(info.heroName));
