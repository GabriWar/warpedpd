...
public class CellSelector extends ScrollArea {
	...
	private float zoom( float value ) {
		...
		WPDSettings.zoom((int) (value - PixelScene.defaultZoom));
		camera.zoom( value );
	...
	public void select( int cell, int button ) {
		...
		//a waypoint march never leaves the hero 'ready' - it seats a fresh leg on every
		//act - so a tap on the world would fall through to cancel() and the march would
		//run on forever. Tapping the world IS how the player says stop: pause the march
		//(the waypoint is kept, the hud tag resumes it) and swallow the tap
		if (cell != -1 && button != PointerEvent.RIGHT && pauseWaypointMarch()) {
			return;
		}
	...
	/** Pauses a running waypoint march and drops the leg the hero is walking.
	 *  Returns true when a march was actually paused. */
	private static boolean pauseWaypointMarch() {
		...
		//the map's own aim calls are flagged, and the tap that placed the waypoint must
		//not leak through the closing window into an instant pause
		...
		//clearing the flag alone only stops NEW legs - the hero would still walk out the
		//current one, up to ten cells past the tap
		Dungeon.hero.interrupt();
		GLog.i( Messages.get( xyz.gabriwar.warpedpixeldungeon.windows.WndWorldMap.class,
				"waypoint_paused" ) );
		GameScene.ready();
		return true;
	}
	...
	//used for movement
	private GameAction heldAction1 = WPDAction.NONE;
	private GameAction heldAction2 = WPDAction.NONE;
	//not used for movement, but helpful if the player holds 3 keys briefly
	private GameAction heldAction3 = WPDAction.NONE;

	private float heldDelay = 0f;
	private boolean delayingForRelease = false;

	private static float initialDelay(){
		switch (WPDSettings.movementHoldSensitivity()){
			case 0:
	...
	private Signal.Listener<KeyEvent> keyListener = new Signal.Listener<KeyEvent>() {
		...
		public boolean onSignal(KeyEvent event) {
			...
			if (!event.pressed){
				...
				if (heldAction1 != WPDAction.NONE && heldAction1 == action) {
					heldAction1 = WPDAction.NONE;
					if (heldAction2 != WPDAction.NONE){
						heldAction1 = heldAction2;
						heldAction2 = WPDAction.NONE;
						if (heldAction3 != WPDAction.NONE){
							heldAction2 = heldAction3;
							heldAction3 = WPDAction.NONE;
						}
					}
				} else if (heldAction2 != WPDAction.NONE && heldAction2 == action){
					heldAction2 = WPDAction.NONE;
					if (heldAction3 != WPDAction.NONE){
						heldAction2 = heldAction3;
						heldAction3 = WPDAction.NONE;
					}
				} else if (heldAction3 != WPDAction.NONE && heldAction3 == action){
					heldAction3 = WPDAction.NONE;
				}
				...
				lastCellMoved = -1;
				if (heldAction1 == WPDAction.NONE){
					heldAction1 = action;
					heldDelay = initialDelay();
					delayingForRelease = false;
				} else if (heldAction2 == WPDAction.NONE){
					heldAction2 = action;
				} else {
					heldAction3 = action;
				}
	...
	private GameAction leftStickAction = WPDAction.NONE;
	...
	public void update() {
		...
		if (newLeftStick != leftStickAction){
			if (leftStickAction == WPDAction.NONE){
				heldDelay = initialDelay();
				Dungeon.hero.resting = false;
			} else if (newLeftStick == WPDAction.NONE && heldDelay > 0f){
				heldDelay = 0f;
	...
	private Point directionFromAction(GameAction action){
		if (action == WPDAction.N)  return new Point( 0, -1);
		if (action == WPDAction.NE) return new Point(+1, -1);
		if (action == WPDAction.E)  return new Point(+1,  0);
		if (action == WPDAction.SE) return new Point(+1, +1);
		if (action == WPDAction.S)  return new Point( 0, +1);
		if (action == WPDAction.SW) return new Point(-1, +1);;
		if (action == WPDAction.W)  return new Point(-1,  0);
		if (action == WPDAction.NW) return new Point(-1, -1);
	...
	public void resetKeyHold(){
		heldAction1 = heldAction2 = heldAction3 = WPDAction.NONE;
	}
