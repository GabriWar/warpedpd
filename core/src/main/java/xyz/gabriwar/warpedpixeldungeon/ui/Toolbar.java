...
import xyz.gabriwar.warpedpixeldungeon.windows.WndQuickBag;
...
public class Toolbar extends Component {
	...
	protected void createChildren() {
		...
		add(new Button(){
			...
			protected void onClick() {
				...
				if (Dungeon.hero != null && Dungeon.hero.ready && !GameScene.cancel()) {
					...
					if (ControllerHandler.controllerActive){
						...
						info += Messages.get(WndKeyBindings.class, WPDAction.LEFT_CLICK.name()) + ": " + Messages.get(Toolbar.class, "quickslot_select") + "\n";
						info += Messages.get(WndKeyBindings.class, WPDAction.RIGHT_CLICK.name()) + ": " + Messages.get(Toolbar.class, "quickslot_assign") + "\n";
			...
			public GameAction keyAction() {
				if (btnWait.active) return WPDAction.QUICKSLOT_SELECTOR;
				else				return null;
			}
		});
		...
		add(btnWait = new Tool(24, 0, 20, 26) {
			...
			protected void onClick() {
				...
				// Player mode: wait must round-trip to host (host owns the hero).
				// Send cell = self.pos — Hero.act() on host treats that as wait/rest.
				if (xyz.gabriwar.warpedpixeldungeon.net.NetManager.isPlayer()
						&& Dungeon.hero != null) {
					xyz.gabriwar.warpedpixeldungeon.net.NetManager.sendPlayerAction(Dungeon.hero.pos);
					return;
				}
				if (Dungeon.hero != null &&  Dungeon.hero.ready && !GameScene.cancel()) {
					examining = false;
			...
			@Override
			public GameAction keyAction() {
				return WPDAction.WAIT;
			}

			@Override
			public GameAction secondaryTooltipAction() {
				return WPDAction.WAIT_OR_PICKUP;
			}
			...
			protected boolean onLongClick() {
				// Player mode: route long-press (full rest) through host. We don't have a
				// "rest until full" wire op, so fall back to a single wait — user can hold
				// or repeat. (TODO: dedicated REST_FULL protocol op if we want true autorest.)
				if (xyz.gabriwar.warpedpixeldungeon.net.NetManager.isPlayer()
						&& Dungeon.hero != null) {
					xyz.gabriwar.warpedpixeldungeon.net.NetManager.sendPlayerAction(Dungeon.hero.pos);
					return true;
		...
		add(new Button(){
			...
			@Override
			public GameAction keyAction() {
				if (btnWait.active) return WPDAction.REST;
				else				return null;
			}
		});
		...
		add(new Button(){
			@Override
			protected void onClick() {
				// Player mode: send cell=self.pos. Host's Hero.act() runs handle() which
				// will pick up a heap if present, else fall back to rest().
				if (xyz.gabriwar.warpedpixeldungeon.net.NetManager.isPlayer()
						&& Dungeon.hero != null) {
					xyz.gabriwar.warpedpixeldungeon.net.NetManager.sendPlayerAction(Dungeon.hero.pos);
					return;
			...
			@Override
			public GameAction keyAction() {
				if (btnWait.active) return WPDAction.WAIT_OR_PICKUP;
				else				return null;
			}
		});
			...
			protected void onClick() {
				...
				if (Dungeon.hero != null && Dungeon.hero.ready) {
					if (!examining && !GameScene.cancel()) {
						...
						// Player mode: search must run on host (host owns the hero state).
						if (xyz.gabriwar.warpedpixeldungeon.net.NetManager.isPlayer()) {
							xyz.gabriwar.warpedpixeldungeon.net.NetManager.sendPlayerActionTyped("search");
						} else {
		...
		add(btnInventory = new Tool(0, 0, 24, 26) {
			...
			protected void onClick() {
				if (Dungeon.hero != null && (Dungeon.hero.ready || !Dungeon.hero.isAlive())) {
					if (WPDSettings.interfaceSize() == 2) {
			...
			protected void createChildren() {
				...
				arrow.visible = WPDSettings.interfaceSize() == 2;
		...
		add(new Button(){
			...
			protected void onClick() {
				if (Dungeon.hero != null && Dungeon.hero.ready && !GameScene.cancel()) {
					...
					if (ControllerHandler.controllerActive){
						...
						info += Messages.get(WndKeyBindings.class, WPDAction.LEFT_CLICK.name()) + ": " + Messages.get(Toolbar.class, "container_select") + "\n";
					...
					Game.scene().addToFront(new RadialMenu(Messages.get(Toolbar.class, "container_prompt"), info, names, images){
						...
						public void onSelect(int idx, boolean alt) {
							...
							if (ControllerHandler.controllerActive){
								...
								info += Messages.get(WndKeyBindings.class, WPDAction.LEFT_CLICK.name()) + ": " + Messages.get(Toolbar.class, "item_select") + "\n";
								info += Messages.get(WndKeyBindings.class, WPDAction.RIGHT_CLICK.name()) + ": " + Messages.get(Toolbar.class, "item_use") + "\n";
			...
			@Override
			public GameAction keyAction() {
				if (btnWait.active) return WPDAction.INVENTORY_SELECTOR;
				else				return null;
			}
		});
	...
	protected void layout() {
		...
		for(int i = startingSlot; i <= endingSlot; i++) {
			if (i == startingSlot && !WPDSettings.flipToolbar() ||
				i == endingSlot && WPDSettings.flipToolbar()){
				btnQuick[i].border(0, 2);
				btnQuick[i].frame(106, 0, 19, 24);
			} else if (i == startingSlot && WPDSettings.flipToolbar() ||
					i == endingSlot && !WPDSettings.flipToolbar()){
				btnQuick[i].border(2, 1);
				btnQuick[i].frame(86, 0, 20, 24);
			} else {
		...
		Toolbar.Mode mode;
		try {
			mode = Mode.valueOf(WPDSettings.toolbarMode());
		} catch (Exception e){
	...
	public static float inventoryBtnRight() {
		if (instance == null) return -1;
		return instance.btnInventory.right();
	}

	public static float inventoryBtnTop() {
		if (instance == null) return -1;
		return instance.btnInventory.top();
	}
