								...
								protected void onSelect(int index) {
									...
									if (index == 0 || index == 1){
										while (Dungeon.hero.talentPointsAvailable(tier) > 0){
											//only pick among talents that can still be raised; with the
											//post-30 shared pool the tier can have points available even
											//when everything here is maxed - without this check the loop
											//would never terminate
											ArrayList<TalentButton> upgradable = new ArrayList<>();
											for (TalentButton b : buttons){
												if (Dungeon.hero.pointsInTalent(b.talent) < b.talent.maxPoints()){
													upgradable.add(b);
												}
											}
											...
											Random.element(upgradable).upgradeTalent();
											if (index == 1){
												break;
											}
										};
