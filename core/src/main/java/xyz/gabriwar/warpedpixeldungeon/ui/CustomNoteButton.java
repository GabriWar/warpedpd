						...
						protected void onSelect(int index) {
							if (index == 0){
								...
								if (parentWindow instanceof WndJournal || parentWindow == null){
									WarpedPixelDungeon.scene().addToFront(new WndJournal());
		...
		GameScene.show(new WndTextInput(promptTitle,
			...
			public void onSelect(boolean positive, String text) {
				if (positive && !text.isEmpty()){
					...
					WarpedPixelDungeon.scene().addToFront(wnd);
					WarpedPixelDungeon.scene().addToFront(new CustomNoteWindow(note, wnd));
				}
			}
		});
	}
