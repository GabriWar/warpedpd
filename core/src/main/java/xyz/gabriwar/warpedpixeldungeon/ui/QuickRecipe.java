...
public class QuickRecipe extends Component {
	...
	public QuickRecipe(Recipe r, ArrayList<Item> inputs, final Item output) {
		...
		for (final Item in : inputs) {
			...
			curr = new ItemSlot(in) {
				...
				protected void onClick() {
					WarpedPixelDungeon.scene().addToFront(new WndInfoItem(in));
				}
			};
		...
		if (hasInputs) {
			...
			if (!(WarpedPixelDungeon.scene() instanceof AlchemyScene)) {
				arrow.enable(false);
			}
		} else {
		...
		this.output = new ItemSlot(output){
			@Override
			protected void onClick() {
				WarpedPixelDungeon.scene().addToFront(new WndInfoItem(output));
			}
		};
	...
	public class arrow extends IconButton {
		...
		protected void onClick() {
			...
			((AlchemyScene)WarpedPixelDungeon.scene()).populate(ingredients, Dungeon.hero.belongings);
		}
	...
	public static ArrayList<QuickRecipe> getRecipes( int pageIdx ){
		...
		switch (pageIdx){
			...
			case 2:
				...
				result.add(null);
				result.add(new QuickRecipe( new StewedMeat.oneMonsterMeat() ));
				result.add(new QuickRecipe( new StewedMeat.twoMonsterMeat() ));
				result.add(new QuickRecipe( new StewedMeat.threeMonsterMeat() ));
				result.add(null);
			...
			case 5:
				...
				result.add(null);
				result.add(new QuickRecipe( new SeekingBombItem.Recipe() ));
				return result;
			case 6:
				...
				result.add(new QuickRecipe( new ArcaneResin.Recipe(),
						...
						new ArcaneResin()));
				result.add(null);
				result.add(new QuickRecipe( new HolyWater.Recipe() ));
			...
			case 8:
				...
				result.add(new QuickRecipe(new SpontaneousCombustion.Recipe()));
				if (!PixelScene.landscape()) result.add(null);
				result.add(null);
