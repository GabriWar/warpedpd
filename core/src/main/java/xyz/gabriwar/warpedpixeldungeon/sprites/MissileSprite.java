...
public class MissileSprite extends ItemSprite implements Tweener.Listener {
	...
	public void reset( PointF from, PointF to, Item item, Callback listener) {
		...
		// setup() mutates from/to in-place (subtracts width/2 etc.) — capture raw
		// coords first so the spectator broadcast carries the same values the host
		// got, not the mutated ones (otherwise the client's setup re-mutates and
		// the missile spawns half-a-tile off + rotates from the wrong origin).
		float rawFx = from.x, rawFy = from.y, rawTx = to.x, rawTy = to.y;
		...
		// Record for spectator sync
		xyz.gabriwar.warpedpixeldungeon.net.NetVisuals.recordMissile(
				rawFx, rawFy, rawTx, rawTy,
				item != null ? item.image : 0,
				angularSpeed, angle);
	}
